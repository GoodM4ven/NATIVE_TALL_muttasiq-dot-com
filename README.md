...
- When testing native server/sync/realtime behavior, run the matching native watch script (`.scripts/watch-android.sh` or `.scripts/watch-ios.sh`). It now starts the native dev stack for you without a second Vite server, then exposes the Laravel API and Reverb over Tailscale and passes the resulting endpoints into the native build. Use plain `composer dev` only when you want the web stack.
